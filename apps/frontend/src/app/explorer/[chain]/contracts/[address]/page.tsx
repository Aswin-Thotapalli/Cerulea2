...
import ContractInteract, { type AbiFunction } from '@/components/explorer/ContractInteract';
import ErrorState from '@/components/explorer/ErrorState';
import { useChainContext } from '@/context/ChainContext';
import { fetchContract } from '@/lib/explorer/api/contracts';
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', px: { xs: 2, sm: 3, md: 4 }, py: 4 }}>
      <Box sx={{ mb: 3, display: 'flex', gap: 2, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        {!loading && !contract?.isVerified && (
          <Button
            component={Link}
            href={`/explorer/${chain}/contracts/verify?address=${address}`}
            variant="outlined"
            size="small"
                ...
                <TableBody>
                  ...
                  {!loading && contract?.deployerAddress && (
                    <DetailRow label="Deployer">
                      <Box component={Link} href={`/explorer/${chain}/accounts/${contract.deployerAddress}`} sx={{ textDecoration: 'none' }}>
