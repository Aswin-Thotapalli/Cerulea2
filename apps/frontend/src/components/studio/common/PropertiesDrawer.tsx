...
function ensureEdgeDefaults(d?: EdgeData): EdgeData {
  ...
  merged.transition = { ...base.transition, ...(d?.transition || {}) };

  merged.retry = { ...base.retry, ...(d?.retry || {}) };
  merged.retry.retryOn = Array.isArray(d?.retry?.retryOn) ? d!.retry!.retryOn : base.retry!.retryOn;

  merged.dataFlow = { ...base.dataFlow, ...(d?.dataFlow || {}) };
  merged.telemetry = { ...base.telemetry, ...(d?.telemetry || {}) };
