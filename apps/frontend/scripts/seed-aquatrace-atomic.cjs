/**
 * Seed: AquaTrace — Cerulea Studio Build Spec (Farmed Aquaculture + Wild Capture)
 ...
 * (flows), Economics (permissioned AquaChain) and Integrations (step-4 draft).
 *
 *   node scripts/seed-aquatrace-atomic.cjs --dry     # validate + print counts, no writes
 *   node scripts/seed-aquatrace-atomic.cjs           # seed (idempotent by slug)
 */
require('dotenv').config({ path: require('path').resolve(__dirname, '../.env.local') });
const crypto = require('crypto');

const S = require('./aquatrace/_shared.cjs');
const { MODULES, EDGES, BLUEPRINT } = require('./aquatrace/modules.cjs');
const { ENTITIES: CORE, RELS: CORE_RELS } = require('./aquatrace/entities-core.cjs');
const { FORMS, FORM_RELS } = require('./aquatrace/forms.cjs');
const { FLOWS } = require('./aquatrace/flows.cjs');
const { ECONOMICS } = require('./aquatrace/economics.cjs');
const { INTEGRATIONS } = require('./aquatrace/integrations.cjs');

const OWNER_EMAIL  = 'aswin@cbytechains.com';
const PROJECT_SLUG = 'aquatrace';
const PROJECT_NAME = 'AquaTrace';
const WORKSPACE    = 'Caerulean Bytechains';
const TEMPLATE_IDS = ['ent-supply-chain'];
const DESCRIPTION  =
  'Seafood traceability for Indian exports on AquaChain (permissioned ledger, ed25519-signed events, SHA-256, ' +
  '5 validators co-signing, 4-of-5 finality, 5 s block interval, no crypto/no gas). Two profiles on one engine: ' +
  'Farmed Aquaculture (broodstock -> hatchery -> pond -> harvest -> plant -> export) and Wild Capture (fishing trip -> ' +
  'landing -> auction -> plant -> export), 28 anchored entities, 30 transaction types, 14 certificate types, 31 roles, ' +
  'two-party custody handoffs, residue/MRL and water-quality monitoring rules, public QR verification with compliance ' +
  'scorecard, nine Indian locales, per-profile terminology and branding. A product of Caerulean Bytechains, built on Cerulea.';
...
const summary = () => {
  console.log('\nAquaTrace — build summary');
  console.log(`  Step 1 Blueprint    : ${MODULES.length} modules, ${EDGES.length} edges`);
  console.log(`  Step 2 Schema       : ${ENTITIES.length} entities (${CORE.length} platform/chain/registry + ${FORMS.length} anchored forms), ${totalFields} fields, ${RELATIONSHIPS.length} relationships`);
  console.log(`  Step 2 Logic        : ${FLOWS.length} flows`);
  console.log(`  Step 3 Economics    : ${ECONOMICS.chain.name} (${ECONOMICS.chain.chainId}) — ${ECONOMICS.chain.CONSENSUS_VALIDATOR_COUNT} validators, ${ECONOMICS.chain.CONSENSUS_THRESHOLD}-of-${ECONOMICS.chain.CONSENSUS_VALIDATOR_COUNT}, ed25519, no token / gasless`);
...
async function main() {
  ...
  console.log('   Open Studio → My Projects → "AquaTrace".');
}
