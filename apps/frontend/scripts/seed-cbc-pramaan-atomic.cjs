/**
 * Seed: CBC-PRAMAAN - Atomic Build List for Cerulea Studio
 ...
 * every Studio step: Blueprint, Schema (entities/fields/relationships), Logic
 * (flows), Economics (non-crypto permissioned chain) and Integrations (step-4 draft).
 *
 *   node scripts/seed-cbc-pramaan-atomic.cjs --dry     # validate + print counts, no writes
 *   node scripts/seed-cbc-pramaan-atomic.cjs           # seed (idempotent by slug)
 */
require('dotenv').config({ path: require('path').resolve(__dirname, '../.env.local') });
const crypto = require('crypto');

const S = require('./cbc-pramaan/_shared.cjs');
const { MODULES, EDGES, BLUEPRINT } = require('./cbc-pramaan/modules.cjs');
const { ENTITIES: CORE, RELS: CORE_RELS } = require('./cbc-pramaan/entities-core.cjs');
const { ENTITIES: PLATFORM, RELS: PLATFORM_RELS } = require('./cbc-pramaan/entities-platform.cjs');
const { FLOWS } = require('./cbc-pramaan/flows.cjs');
const { ECONOMICS } = require('./cbc-pramaan/economics.cjs');
const { INTEGRATIONS } = require('./cbc-pramaan/integrations.cjs');

const OWNER_EMAIL  = 'aswin@cbytechains.com';
const PROJECT_SLUG = 'cbc-pramaan';
const PROJECT_NAME = 'CBC-PRAMAAN';
const WORKSPACE    = 'Caerulean Bytechains';
const TEMPLATE_IDS = ['govt-procurement'];
const DESCRIPTION  =
  'Cerulea Bytechains Compliance, Procurement Record And Make-in-India Assurance Network: a blockchain-based PPP-MII ' +
  'compliance trust layer behind GeM. Non-crypto, permissioned (no tokens, wallets or mining) on a hash-chained, ' +
  'DCF quorum-finalised (2 of 3 validators), ACL-scoped Cerulea private chain. Six stakeholder roles, 14 identities, ' +
  'nine transaction types, a per-HSN Rule Registry with the DPIIT floor enforced, five deterministic engines ' +
  '(Classification, Consistency, Debarment, Certification, Purchase Preference), the six GeM trigger points plus read ' +
  'and explorer endpoints, nine screens, deterministic seed (26 blocks) and 12 acceptance scenarios. ' +
  'Live PoC at cbc-pramaan.cerulea.io. A product of Caerulean Bytechains, built on Cerulea.';

/* ---------------------------- Assemble ---------------------------- */
const ENTITIES = [...CORE, ...PLATFORM];
const byName = new Map();
for (const e of ENTITIES) {
  if (byName.has(e.name)) throw new Error(`Duplicate entity name: ${e.name}`);
  byName.set(e.name, e);
}
const RELATIONSHIPS = [...CORE_RELS, ...PLATFORM_RELS].map(([from, to, type], i) => {
...
const summary = () => {
  console.log('\nCBC-PRAMAAN - build summary');
  console.log(`  Step 1 Blueprint    : ${MODULES.length} modules, ${EDGES.length} edges`);
  console.log(`  Step 2 Schema       : ${ENTITIES.length} entities (${CORE.length} data/chain + ${PLATFORM.length} platform/screens/ops), ${totalFields} fields, ${RELATIONSHIPS.length} relationships`);
  console.log(`  Step 2 Logic        : ${FLOWS.length} flows`);
  console.log(`  Step 3 Economics    : ${ECONOMICS.chain.name} (${ECONOMICS.chain.chainId}) - ${ECONOMICS.chain.CONSENSUS_VALIDATOR_COUNT} validators, DCF ${ECONOMICS.chain.CONSENSUS_THRESHOLD}-of-${ECONOMICS.chain.CONSENSUS_VALIDATOR_COUNT}, no token / gasless`);
...
async function main() {
  ...
  console.log('   Open Studio -> My Projects -> "CBC-PRAMAAN".');
}
