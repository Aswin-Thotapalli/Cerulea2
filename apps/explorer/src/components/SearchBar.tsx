...
function classifyQuery(q: string): QueryType {
  ...
  // 32-byte hex (64 chars) with or without 0x prefix — tx hash or block hash
  if (/^(0x)?[0-9a-fA-F]{64}$/.test(trimmed)) return 'tx';
  // EVM address: 0x + 40 hex chars
  if (/^0x[0-9a-fA-F]{40}$/.test(trimmed)) return 'account';
  // SS58 address: base58 encoded, typically 47-48 chars starting with 1, 2, 5, etc.
...
export default function SearchBar({ placeholder = 'Search by block / tx hash / address', fullWidth }: SearchBarProps) {
  ...
  const navigate = useCallback((q: string, hint?: QueryType) => {
    ...
    switch (type) {
      case 'block':
        router.push(`/${chain}/blocks/${trimmed}`);
        break;
      case 'tx':
        router.push(`/${chain}/txs/${trimmed}`);
        break;
      case 'contract':
        router.push(`/${chain}/contracts/${trimmed}`);
        break;
      case 'account':
        router.push(`/${chain}/accounts/${trimmed}`);
        break;
      default:
        // Treat as tx hash first (most common free-text search)
        router.push(`/${chain}/txs/${trimmed}`);
    }
  }, [chain, router]);
